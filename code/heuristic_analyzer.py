...
ISSUE_TYPE_KEYWORDS = {
    "crack": [
        ...
        "shattered", "shattered screen", "screen shatter",   # screen shatter = crack
        "glass shatter"
    ],
    "dent": [
...
OBJECT_PART_KEYWORDS = {
    ...
    "laptop": {
        "screen": [
            ...
            "screen shatter", "shattered screen"
        ],
        "hinge": [
...
def extract_issue_type(conversation, claim_object):
    ...
    if claim_object == "package":
        ordered = [
            "water_damage",
            "torn_packaging",    # torn before crushed (more specific wording)
            "crushed_packaging",
            "stain", "crack", "broken_part", "scratch", "dent", "none"
        ]
    else:
        ordered = [
            "water_damage", "stain", "crack", "scratch",
            "broken_part", "dent", "none"
        ]
...
def analyze_claim_heuristic(image_paths_with_ids, user_claim, claim_object):
    ...
    # 3. Laptop + crack → object_part is almost always screen
    if claim_object == "laptop" and issue_type == "crack":
        object_part = "screen"
